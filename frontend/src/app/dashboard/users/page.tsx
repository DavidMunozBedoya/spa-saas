...
export default function UsersPage() {
  ...
  useEffect(() => {
    try {
      ...
      if (userDataStr) {
        ...
        else if (userData.isPlatformAdmin || userData.role === "SUPER_ADMIN") {
          setUserPermissions(["platform:manage", "staff:manage", "services:manage", "users:manage"]);
        }
      }
    } catch (error) {
      console.error("Token decoding or permission error", error);
    }
  }, []);
  ...
  const hasManageUsers = userPermissions.includes("users:manage") || userPermissions.includes("platform:manage");
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground-auto mt-1 font-medium">
            Gestiona quién tiene acceso a la plataforma y qué puede ver.
          </p>
        </div>
            ...
            <table className="w-full text-left border-collapse whitespace-nowrap">
              <thead>
                <tr className="border-b border-foreground/10 bg-foreground/5 text-xs uppercase tracking-widest text-foreground/50">
                  <th className="p-5 font-bold pl-6">Usuario (Credencial)</th>
                  <th className="p-5 font-bold">Perfil Vinculado</th>
                  <th className="p-5 font-bold">Rol Dominante</th>
                  <th className="p-5 font-bold text-right pr-6">Acciones</th>
                </tr>
              </thead>
                    ...
                    <tr key={user.id} className="hover:bg-foreground/5 transition-colors group">
                      <td className="p-5 pl-6">
                        <div className="min-w-0">
                          ...
                          <p className="text-xs text-foreground/50 font-mono mt-0.5">{user.email}</p>
                        </div>
                      </td>
                      
                      <td className="p-5">
                        {user.staff_id ? (
                          <div className="inline-flex items-center gap-2 px-2.5 py-1 bg-foreground/5 border border-foreground/10 rounded-lg text-xs font-bold text-foreground/80">
                            <UserCheck size={14} className="text-primary" />
                            Staff Asociado
                          </div>
                        ) : (
                          <span className="text-foreground/40 text-xs italic flex items-center gap-1">
                            <ShieldAlert size={12} />
                            Sin vincular
                          </span>
                        )}
                        ...
                        {hasManageUsers && (
                          <div className="flex justify-end gap-2 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
                            ...
                            <button 
                              ...
                              className="flex items-center gap-1.5 px-3 py-1.5 text-xs bg-foreground/5 hover:bg-foreground/10 text-foreground font-bold rounded-lg border border-foreground/10 transition-colors"
