...
export default function StaffPage() {
  ...
  const hasManageStaff = userPermissions.includes("staff:manage") || userPermissions.includes("platform:manage");
  ...
  const filteredStaff = staffList.filter(s => {
    ...
    // Si showInactive es false, solo mostramos los activos. Si es true, mostramos TODO.
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground-auto mt-1 font-medium">
            Administra a los terapeutas, estilistas y empleados de tu negocio.
          </p>
        </div>
            ...
            <table className="w-full text-left border-collapse whitespace-nowrap">
              <thead>
                <tr className="border-b border-foreground/10 bg-foreground/5 text-xs uppercase tracking-widest text-foreground/50">
                  <th className="p-4 font-bold pl-6">Empleado</th>
                    ...
                    <td className="p-4">
                      <div className="flex flex-col gap-1 text-sm">
                        ...
                        {!staff.email && !staff.phone && (
                          <span className="text-foreground/40 text-xs italic">Sin contacto registrado</span>
                        )}
                      </div>
                    </td>
                    <td className="p-4">
                        ...
                        <div className="flex items-center justify-end gap-2 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
                          {!staff.active ? (
                            <button 
                              ...
                              className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-500/10 text-emerald-500 hover:bg-emerald-500/20 rounded-lg transition-colors text-xs font-bold" 
                              title="Reactivar"
                            >
                              <RefreshCcw size={14} />
                              Reactivar
